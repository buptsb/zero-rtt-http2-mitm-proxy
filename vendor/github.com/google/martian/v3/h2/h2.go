...
func (c *Config) Proxy(closing chan bool, cc io.ReadWriteCloser, url *url.URL) error {
	defer cc.Close()
	...
	if c.DialServerConn != nil {
		sc, err = c.DialServerConn(url.Host)
	} else {
	...
	wg.Add(2)
	shouldCloseCh := make(chan struct{}, 2)
